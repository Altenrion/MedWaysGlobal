...
class EDataTables extends CGridView
{
	...
	public function registerClientScript()
	{
		...
		Yii::app()->getClientScript()->registerScript(__CLASS__.'#'.$id, "jQuery('#$id').eDataTables($options);",CClientScript::POS_END);
	}
...
//		$cs->registerCoreScript('jquery');
		...
		$cs->registerScriptFile($baseScriptUrl.'/js/jquery.dataTables'.(YII_DEBUG ? '' : '.min' ).'.js' ,CClientScript::POS_END);
		if ($configurable || $registerJUI) {
			$cs->registerCoreScript('jquery.ui');
		}
		if ($configurable) {
			$cs->registerScriptFile($baseScriptUrl.'/js/ColReorder'.(YII_DEBUG ? '' : '.min' ).'.js',CClientScript::POS_END);
		}
		foreach($jsFiles as $key=>$value) {
			...
			$cs->registerScriptFile((strpos($jsFile,'/')===false ? $baseScriptUrl.'/js/' : '').$jsFile,CClientScript::POS_END);
		}
	}
